...
[@snajafov](https://github.com/snajafov),
...
- [#1674](https://github.com/InvoicePlane/InvoicePlane/pull/1674) — **CSRF (state-changing GET on PDF generation):** with the "mark as sent when generating the PDF" option enabled (`mark_invoices_sent_pdf` / `mark_quotes_sent_pdf`), `invoices/generate_pdf/<id>` and `quotes/generate_pdf/<id>` changed document state on a plain authenticated GET before streaming the file — assigning the next official invoice/quote number, moving the document from draft to *sent*, updating its due date, and (where the read-only option is set) locking it. Any page an administrator merely visited could therefore fire these changes with a hidden `<img src=".../invoices/generate_pdf/ID">`, consuming official numbers and pushing drafts to sent with no action by the admin. The state changes now run only when the request carries a valid same-origin CSRF token: a new `verify_get_csrf_token()` compares an `_ip_csrf` query-string token against the CSRF cookie with `hash_equals()` (the query-string counterpart of the existing POST-only `verify_csrf_token()`), and the in-app "generate PDF" links attach the token through a new `_csrf_query()` helper. A forged cross-site request can't read the cookie, so it can't supply a matching token and the side effect is skipped — while the PDF, a read-only operation, still streams either way, leaving every legitimate download link and the auto-mark-sent workflow working exactly as before. Thanks to [@ashrexon](https://github.com/ashrexon) (Yash Shendge) for the responsible disclosure.
...
- [#1677](https://github.com/InvoicePlane/InvoicePlane/issues/1677) — **Stripe zero-decimal currencies:** Checkout Sessions now convert invoice balances using the configured currency's minor unit instead of multiplying every amount by 100. Callback validation, recorded payments, and merchant-response details use the same conversion, preventing JPY, KRW, CLP, and other zero-decimal invoices from charging customers 100 times the invoice total. Thanks to [@snajafov](https://github.com/snajafov) for reporting the issue.
