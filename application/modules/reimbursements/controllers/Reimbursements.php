...
class Reimbursements extends Admin_Controller
{
    ...
    public function delete($reimbursement_id = null)
    {
        check_permission('reimbursements', 'delete');
        ...
        $is_admin = has_permission('reimbursements', 'edit');
        if (!$is_admin && (int) $reimbursement->user_id !== (int) $this->session->userdata('user_id')) {
            $this->session->set_flashdata('alert_error', 'Anda tidak memiliki izin menghapus klaim ini.');
            redirect('reimbursements');
        }

        if ($reimbursement->status !== 'pending' && !$is_admin) {
            $this->session->set_flashdata('alert_error', 'Hanya klaim berstatus pending yang dapat dihapus.');
            redirect('reimbursements');
        }
        ...
        $this->mdl_reimbursements->delete($reimbursement_id);
        $this->session->set_flashdata('alert_success', 'Klaim reimburse berhasil dihapus.');
        redirect('reimbursements');
    }
}
